...
class ScopeInterface(ABC):
    """Store and retrieve values within one runtime scope.
    ...
    def get_values(self, key: Hashable) -> tuple[object, ...]:
        ...
        return tuple(record.value for record in self.get_records(key))

    def get_record(self, key: Hashable) -> ValueRecord | None:
        """Return the most recent full record for ``key``, or ``None``."""
        records = self.get_records(key)
